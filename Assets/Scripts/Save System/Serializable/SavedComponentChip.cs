...
public class SavedComponentChip
{
    ...
    public SavedComponentChip(ChipSaveData chipSaveData, Chip chip)
    {
        ...
        for (var i = 0; i < inputPins.Length; i++) inputPins[i] = new SavedInputPin(chipSaveData, chip.inputPins[i]);
        ...
        for (var i = 0; i < chip.outputPins.Length; i++) outputPinNames[i] = chip.outputPins[i].pinName;
    }
}